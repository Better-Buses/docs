...
== Layers
=== Edge Layer
The primary role of the bus-mounted sensors is localized data ingestion. They act as lightweight edge devices responsible for continuously collecting raw transit data (such as GPS coordinates, time stamps, and speed variations) without overloading local processing capacity.
Instead those mounted in the stops serves like a checkpoint station in order to track automatically at which stop and what what time a bus arrived.
== Fog Layer
...
- *Fog Layer Emulation*: Dedicated VMs are configured to simulate the local fog nodes like specified in the Layers section
...
=== Kubernetes
To manage our data-processing applications within the fog layer, we deploy a Kubernetes cluster directly on top of the virtualized infrastructure provisioned by OpenNebula. This container orchestration strategy separates the infrastructure into a centralized Control Plane and a resilient Worker Layer.
A single, dedicated OpenNebula virtual machine is allocated to act exclusively as the Kubernetes Control Plane (Master Node). This node serves as the brain of the cluster, executing core components such as scheduler, and controller manager.
The other VMs are configured as Kubernetes Worker Nodes. These nodes will host the application workloads encapsulated within Pods. These worker pods execute the microservices responsible for real-time data processing and filtering.
By leveraging Kubernetes on top of our fog nodes, the system inherits robust self-healing mechanisms to ensure uninterrupted data streams and guarantees Pod replication and Workload rescheduling.
=== Grafana
...
=== Security
For the security aspect and protect the integrity of the transit data and the stability of the infrastructure we decided to adopt two main level of security:
==== Container Hardening
